...
const renderResults = (results) => {
    ...
    for (const result of results) {
        ...
        const base = hashIndex >= 0 ? permalink.slice(0, hashIndex) : permalink;
        const hash = hashIndex >= 0 ? permalink.slice(hashIndex) : '';
        const sep = base.includes('?') ? '&' : '?';
        link.href = base + (currentQuery ? sep + 'q=' + encodeURIComponent(currentQuery) : '') + hash;
...
const performSearch = () => {
    ...
    highlightResultSnippets(results, query);
};

const highlightResultSnippets = (results, query) => {
    if (!Array.isArray(results) || !resList) return;
    const lis = resList.querySelectorAll('li');
    lis.forEach((li, i) => {
        const result = results[i];
        if (!result) return;
        const content = (result.item.content || '').replace(/<[^>]+>/g, '');
        const idx = content.toLowerCase().indexOf(query.toLowerCase());
        let snip = '';
        if (idx >= 0) {
            const start = Math.max(0, idx - 20);
            snip = (start > 0 ? '…' : '') + content.slice(start, idx + query.length + 40).trim();
        } else {
            snip = content.slice(0, 60).trim();
        }
        const mark = document.createElement('span');
        mark.className = 'search-snippet';
        mark.textContent = snip + (snip.length >= 60 ? '…' : '');
        li.appendChild(mark);
    });
};
